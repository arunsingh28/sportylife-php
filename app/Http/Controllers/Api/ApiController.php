...
class ApiController extends Controller
{
    ...
    public function userDetails(Request $request)
    {
        try {
            ...
            $Settings = Settings::where('type','profile_comming_soon_image')->first();
            if ($user) {
                ...
                $user->profile_comming_soon_image = @$Settings->value;
            ...
            foreach ($order_data as $key => $value) {
                ...
                if (!empty($order_items[0])) {
                    foreach ($order_items as $key1 => $value1) {
                        ...
                        $value1->start_date = date("d/m/Y",strtotime($get_start_date[0]['start_date']));
    ...
    public function getUserPackageIOS()
    {
        ...
        foreach ($order_data as $key => $value) {
            ...
            //             $order_package_items = User_order_items::where('order_primary_id', $value->id)->where('package_id', $index)->orderBy("category_year_srno", "desc")->get();
            //             $get_start_date = User_order_items::where('order_primary_id', $value->id)->where('package_id', $index)->orderBy("category_year_srno", "asc")->get();
            //             $value->start_date = date("d/m/Y",strtotime($get_start_date[0]['start_date']));
            //             $value->expiry_date = date("d/m/Y",strtotime($order_package_items[0]['end_date']));
            //         }
            //     }
        }
    ...
    public function profileUpdate(Request $request)
    {
        try {
            ...
            $input = $request->only('first_name', 'last_name', 'gender', 'language_id', 'city', 'state', 'weight', 'weight_type', 'height_type', 'height_feet', 'height_inch', 'school_name', 'school_address','school_unique_id','zipcode','name');
            $input['dob'] = date("Y-m-d", strtotime($request->dob));
    ...
    public function logout(Request $request)
    {
        try {
            if (auth()->check()) {
                $update_device_token = User::where('user_type', "user")->where('id', auth()->user()->id)->update(['device_token' => NULL]);
    ...
    public function addtoCart(Request $request)
    {
        try {
            ...
            if ($request->type == "sport") {
                ...
                $check_existing_order = User_orders::where('user_id', $request->user_id)
                ...
                if (!empty($check_existing_order)) {
                    ...
                    if (!empty($order_package_items[0])) {
                        if (date('Y-m-d') < date('Y-m-d',strtotime($order_package_items[0]['end_date']))) {
                            return response()->json(['statusCode' => 999, 'message' => 'Already purchased!']);
                        }
                    }else{
                        return response()->json(['statusCode' => 999, 'message' => 'Already purchased!']);
                    }
        ...
        try {
            ...
            if ($request->type == "sport") {
                $check_existing_order = User_orders::where('user_id', $request->user_id)
                                                ...
                                                ->first();
                if (!empty($check_existing_order)) {
                    $order_package_items = User_order_items::where('user_id',$request->user_id)->where('order_primary_id', $check_existing_order->id)->orderBy("category_year_srno", "desc")->get();
                    if (!empty($order_package_items[0])) {
                        if (date('Y-m-d') < date('Y-m-d',strtotime($order_package_items[0]['end_date']))) {
                            return response()->json(['statusCode' => 999, 'message' => 'Already purchased!']);
                        }
    ...
    public function razorpayPaymentcallback(Request $request)
    {
        ...
        if (!empty($orderdata)) {
            ...
            @$package_data = Servicepackages::where('id', $orderdetail->package_id)->first();
            ...
            $mail_res = \Mail::send('sendPlanPurchase',
                ['message_data' => $message_data,'name' => $user->name,'subject' => @$request->subject,'package_title' => @$package_data->title,'package_duration' => @$package_data->package_duration,'duration_type' => @$package_data->duration_type,'purchase_games' => @$purchase_games],
                function ($message) use ($email, $subject) {
                    $message->to($email)
                        ->subject($subject);
                }
    ...
    {
        $check_orderdata = User_orders::where('order_id', $request->order_id)->first();
        \Log::info(@$request->order_id);
        \Log::info(@$request->ios_package_id);
        \Log::info(@$request->ios_inapp_id);
        \Log::info(@$request->ios_purchase_date);
        
        if (!empty($check_orderdata)) {
            $orderdata = User_orders::where('order_id', $request->order_id)->update(['ios_package_id' => @$request->ios_package_id, 'ios_inapp_id' => @$request->ios_inapp_id, 'ios_purchase_date' => @$request->ios_purchase_date, 'payment_status' => "complete"]);
    ...
    public function getRecordedVideo(Request $request)
    {
        try {
            $rules = [
            ];
            ...
            if (!empty($data[0])) {
                ...
                foreach ($data as $key => $value) {
                    // if ($value->id == '6') {
                        $recorded_video = Live_videos::where('category_id', $value->id)->where('status', '1')->whereDate('end_date_time', '<=', $currentdate_time)->select('id', 'category_id', 'title', 'thumbnail', 'video', 'status', 'start_date_time', 'created_at', 'updated_at')->get();
                        if (empty($recorded_video[0])) {
                            $value->workoutvideodata = [];
                        }
                        foreach ($recorded_video as $key1 => $value1) {
                            $livetime = Carbon::parse($value1->start_date_time)->addMinutes(30);
                            if ($livetime <= $currentdate_time) {
                ...
                // $data = array_merge($data->toArray(), $recorded_video->toArray());
                return response()->json(['statusCode' => 200, 'message' => 'Recorded Video List Available.', 'data' => $arr]);
            }
            return response()->json(['statusCode' => 999, 'message' => 'No Workout Video.']);
        } catch (Exception $e) {
            return response()->json(['statusCode' => 999, 'message' => $e->getMessage()]);
        ...
        try {
            ...
            if (!empty($user)) {
                if ($user->forceDelete()) {
                    ...
                    return response()->json(['statusCode' => 200, 'message' => 'Remove Successfully.', 'data' => []]);
                } else {
                    return response()->json(['statusCode' => 999, 'message' => 'Error! Please Try Again.']);
                }
            }else{
                return response()->json(['statusCode' => 999, 'message' => 'User Not Found.']);
            }
    ...
    public function updatePackagePayment(Request $request)
    {
        try {
            ...
            $arr = ['order_KOUxaAVwpxf6Wb','order_KOUaEVA7opLsU7','order_KOU4zShw1jx8gn','order_KMCXSp15M3hgG3','order_KHjLf39kcndSnJ','order_KHjKwVaJve2zLy','order_KHjJc17MlnZlYJ','order_KHjK6Z7AB5DXz5','order_KHjJq2wFEthrmY','order_KGtNQP8doLnNxI','order_KGBHXqNSiii09z','order_KG574JyvJMarML','order_KFm3v4LK6x8vUm','order_KFlaq2dqEEWCyJ','order_KFlSbQKGrqkY5T','order_KFlFQmgS7Qnl9J','order_KFl8bMMPwkk6Gb','order_KFl52zKtUbvYD4','order_KFl1ETGa9lhkwe','order_KCZxdkgXu7BgBk','order_KCZq278l66crHY','order_KCZjRGMX1VzB7C','order_KAcRbPy7OIF5UK','order_KA6GnAdSUbG8Jo','order_K6yLb504zzpFr0','order_K6fFhh1yus0EGk','order_K3VbVinxeqPPbs','order_K3QHgTKdTiDRxA'];
            
            $data = User_orders::whereIn('razorpay_orderid', $arr)->get('user_id');
            if (!empty($data[0])) {
                // foreach ($data as $key => $value) {
                //     // $item_data = User_order_items::where('order_primary_id', $value->id)->whereDate('end_date', '>=', date('Y-m-d'))->update(['status'=>'1']);
                //     // $item_data = User_order_items::where('order_primary_id', $value->id)->whereDate('end_date', '>=', date('Y-m-d'))->get();
                // }
                return response()->json(['statusCode' => 200, 'message' => 'Update.', 'data' => $data]);
            }
            return response()->json(['statusCode' => 999, 'message' => 'No Data Found!']);
        } catch (Exception $e) {
            return response()->json(['statusCode' => 999, 'message' => $e->getMessage()]);
        }
    }
