            ...
            <div class="card">
                <div class="card-header border-0">
                    <div class="row align-items-center">
                        <div class="col-8">
                            <h3 class="mb-0">Nutrition Diet Frequency List </h3>
                        </div>
                        <div class="col-4 text-right">
                            @if(auth()->user()->role_id == "1" || auth()->user()->role_id == "10") 
                            <a href="{{route('nutri-diet-frequency-add')}}"
                    ...
                    <table class="table align-items-center table-flush" id="myTable">
                        ...
                        @foreach($data as $key => $item )
                            <tr>
                                ...
                                <td>
                                    @if(auth()->user()->role_id == "1" || auth()->user()->role_id == "4" || auth()->user()->role_id == "10" || $roles_data->type == "new") 
                                    <a href="{{url('/admin/nutri-diet-frequency-edit',$item->id)}}"
                                    class="btn btn-sm btn-primary">Edit</a>
                                    @endif
                                    @if(auth()->user()->role_id == "1" || auth()->user()->role_id == "10" || $roles_data->type == "new") 
                                    <a href="{{url('/admin/nutri-diet-frequency-delete',$item->id)}}"
        ...
        $(function () {
            $('#myTable').on('change', 'tbody input.changestatus', function () {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{url("/admin/change-nutri-diet-frequency-status")}}',
