            ...
            <div class="card">
                <div class="card-header border-0">
                    <div class="row align-items-center">
                        <div class="col-8">
                            <h3 class="mb-0">Nutrition Recipe Categories List </h3>
                        </div>
                        <div class="col-4 text-right">
                            @if(auth()->user()->role_id == "1" || auth()->user()->role_id == "10" || $roles_data->type == "new") 
                            <a href="{{route('nutri-recipe-categories-add')}}"
                        ...
                        @foreach($data as $key => $item )
                            <tr>
                                ...
                                <td>
                                    ...
                                    <img alt="{{$item->title}}" src="{{asset($item->image)}}" width="100px">
                                    </a>
                                </td>
                                <th>
                                    {{$item->created_at}}
                                ...
                                <td>
                                    @if(auth()->user()->role_id == "1" || auth()->user()->role_id == "4" || auth()->user()->role_id == "10" || $roles_data->type == "new") 
                                    <a href="{{url('/admin/nutri-recipe-categories-edit',$item->id)}}"
                                       class="btn btn-sm btn-primary">Edit</a>
                                    @endif
                                    @if(auth()->user()->role_id == "1" || auth()->user()->role_id == "10" || $roles_data->type == "new") 
                                    <a href="{{url('/admin/nutri-recipe-categories-delete',$item->id)}}"
        ...
        $(function () {
            $('#myTable').on('change', 'tbody input.changestatus', function () {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{url("/admin/change-nutri-recipe-categories-status")}}',
