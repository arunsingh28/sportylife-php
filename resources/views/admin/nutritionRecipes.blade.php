      ...
      <div class="card">
         <div class="card-header border-0">
            <div class="row align-items-center">
               <div class="col-8">
                  <h3 class="mb-0">Nutrition Recipe's List </h3>
               </div>
               <div class="col-4 text-right">
                  @if(auth()->user()->role_id == "1"  || auth()->user()->role_id == "10") 
                  <a href="{{route('nutrition-recipe-add')}}" class="btn btn-sm btn-default float-right">Add</a>
                  @endif
               </div>
            </div>
         </div>
            ...
            <table class="table align-items-center table-flush" id="myTable">
               ...
               <tbody class="list">
                  ...
                  <tr>
                     ...
                     <td>
                        {{$item->recipe_categorydata->title ?? ''}}
                     </td>
                     ...
                     <td>
                        @if(auth()->user()->role_id == "1" || auth()->user()->role_id == "4" || auth()->user()->role_id == "10") 
                        <a href="{{url('/admin/nutrition-recipe-edit',$item->id)}}" class="btn btn-sm btn-primary">Edit</a>
                        @endif
                        @if(auth()->user()->role_id == "1" || auth()->user()->role_id == "10") 
                        <a href="{{url('/admin/nutrition-recipe-delete',$item->id)}}" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this?');"> Delete </a>
                        @endif
                     </td>
                  </tr>
                  <?php $i++;?>
...
$(function() {
    $('#myTable').on('change', 'tbody input.changestatus', function() {
        ...
        $.ajax({
            type: "GET",
            dataType: "json",
            url: '{{url("/admin/changeNutritionRecipeStatus")}}',
