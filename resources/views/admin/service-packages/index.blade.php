            ...
            <div class="card">
                <div class="card-header border-0">
                    <div class="row align-items-center">
                        <div class="col-8">
                            <h3 class="mb-0">Service Packages List </h3>
                        </div>
                        <div class="col-4 text-right">
                            <a href="{{route('service-packages-add')}}"
                    ...
                    <table class="table align-items-center table-flush" id="myTable">
                        ...
                        @foreach($data as $key => $item )
                            <tr>
                                ...
                                <td>
                                    @if($item->parent_id == NULL)
                                    <input type="checkbox" class="changestatus_sportgrayout" data-id="{{$item->id}}" data-on="Active" data-off="InActive" {{ $item->is_sports_show == 0 ? 'checked' : '' }}>
                                    @endif
                                </td>
                                <td>
                                    <a href="{{url('/admin/service-packages',$item->id)}}" class="btn btn-sm btn-success">View</a>
                                    @if(auth()->user()->role_id != "3" && auth()->user()->role_id != "4" ) 
                                    <a href="{{url('/admin/service-packages-edit',$item->id)}}" class="btn btn-sm btn-primary">Edit</a>
                                    @endif
                                    {{--@if($item->id > 4)
                                    <a href="{{url('/admin/service-packages-delete',$item->id)}}"
...
@section('script')
    <script>
        ...
        $(function () {
            $('#myTable').on('change', 'tbody input.changestatus_sportgrayout', function () {
                var is_sports_show = $(this).prop('checked') == true ? 0 : 1;
                var id = $(this).data('id');
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{url("/admin/changeStatusSportShow")}}',
                    data: {'is_sports_show': is_sports_show, 'id': id},
                    success: function (data) {
                        console.log(data.success)
                    }
                });
