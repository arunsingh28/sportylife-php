               ...
               <form role="form" method="POST" action="{{ url('orderPlace') }}">
                  ...
                  <div class="row">
                     ...
                     <!-- @if(!empty($data[0]['categorydata']))
                        @if($data[0]['packagedata']['addon_adult_count'] > 0 || $data[0]['packagedata']['addon_kid_count'] > 0 )
                           @if($data[0]['packagedata']['remaining_adult_count'] > 0 || $data[0]['packagedata']['remaining_kid_count'] > 0 )
                           <div class="col-md-12">
                              <div class="row">
                                 <div class="col-md-6" style="margin-left: 26% !important;">
                                    <input type="button" style="margin: 0px auto 17px !important;" class="btn-send"   value="Add Person" data-toggle="modal" data-target="#personaddon">
                                 </div>
                              </div>
                           </div>
                           @else
                           <div class="col-md-12">
                              <div class="row">
                                 <div class="col-md-6" style="margin-left: 26% !important;">
                                    <input type="button" style="margin: 0px auto 17px !important;" class="btn-send"   value="Add Person" >
                                 </div>
                              </div>
                           </div>
                           @endif
                        @endif
                     @endif -->
                     <!-- @if(!empty($addonperson[0])) -->
                     <!-- <div class="col-md-12 Product_del">
                        <h5 style="padding-bottom: 0px !important;">Person's List</h5>
                        <ul class=" list-part">
                           <li class="list-group-item">
                              <label>Note: You can add total {{$data[0]['packagedata']['addon_adult_count']}} Adult and {{$data[0]['packagedata']['addon_kid_count']}} Kid</label>
                              <label style="color:red;">Remaining: You can add total {{$data[0]['packagedata']['remaining_adult_count']}} Adult and {{$data[0]['packagedata']['remaining_kid_count']}} Kid</label>
                           </li>
                        </ul>
                        <hr >
                        <ul class=" list-part">
                           @foreach($addonperson as $key => $item)
                           <li class="list-group-item">
                              <label>{{$key+1}}. {{$item->person_first_name}}</label>  <span ><i style="float: right;" class="fa fa-trash" onclick="removePerson(<?php echo $item->id; ?>)"></i></span>
                           </li>
                           @endforeach
                        </ul>
                     </div> -->
                     ...
                     <div class="col-md-12 Product_del mt-2">
                        ...
                        <input name="click_type" type="hidden"  value="cart">                         
                        ...
                        <ul class=" list-part">
                           ...
                           <div id="sort_menu{{$key1+1}}" data-sort_menu_id="{{$item1['id']}}" class="sort_menu mb-3"  style="border: 1px solid gray !important;padding: 13px !important;border-radius: 11px !important;">
                              ...
                              <li class="list-group-item"  data-id="{{$item->id}}">
                                 <label class="">{{$key+1}}. {{$item->title}}</label>  
                                 <i data-p_id="{{$item1['id']}}" class="fa fa-arrows rearrange mt-1" style="float: right;" aria-hidden="true"></i>
                              ...
                              @if($item1->packagedata['remaining_adult_count'] > 0 || $item1->packagedata['remaining_kid_count'] > 0 )
                              <hr>
                                 <div class="col-md-12" >
                                    <div class="row">
                                       <div class="col-md-6" style="margin-left: 25% !important;">
                                          <input type="button" style="margin: 20px 50px 0 !important;background: #373737 !important;border-radius: 10px !important;" class="btn-send"   value="Add Person" data-toggle="modal" data-target="#personaddon">
                                       </div>
                                    </div>
                                 </div>
                                 @else
                                 <div class="col-md-12">
                                    <div class="row">
                                       <div class="col-md-6" style="margin-left: 25% !important;">
                                          <input type="button" style="margin: 20px 50px 0 !important;background: #373737 !important;border-radius: 10px !important;" class="btn-send"   value="Add Person" >
                                       </div>
                                    </div>
                                 </div>
                                 @endif
                              ...
                              <div class="col-md-12 Product_del" style="margin-top: -21px !important;">
                                 <div class="row">
                                    ...
                                    <div class="col-md-6 mt-2">
                                       @if($item1->packagedata['addon_adult_count'] > 0 || $item1->packagedata['addon_kid_count'] > 0 )
                                       <span class="badge" style="color: white;border-radius: 4px;background: #373737;padding: 8px;font-weight: 400;float: right;" data-toggle="modal" data-target="#personaddon{{$item1['package_id']}}">Add Person</span>
                                       <!-- @if($item1->packagedata['remaining_adult_count'] > 0 || $item1->packagedata['remaining_kid_count'] > 0 )
                                          @else
                                 ...
                                 <!-- <ul class=" list-part mt-4">
                                    <li class="list-group-item">
                                       <label>Note: You can add total {{$item1->packagedata['addon_adult_count']}} Adult and {{$item1->packagedata['addon_kid_count']}} Kid</label><br>
                                       <label style="color:red;">Remaining: You can add total {{$item1->packagedata['remaining_adult_count']}} Adult and {{$item1->packagedata['remaining_kid_count']}} Kid</label>
                                    </li>
                                 </ul> -->
                                 <hr class="mt-1">
                                 <ul class=" list-part" id="listhtml{{$item1['package_id']}}">
                                    @if(!empty($item1->addonperson[0]))
                                    @foreach($item1->addonperson as $key => $item)
                                    <li class="list-group-item">
                                       <label>{{$key+1}}. {{$item->person_first_name}}</label>  <span ><i style="float: right;" class="fa fa-trash" onclick="removePerson(<?php echo $item->id; ?>,<?php echo $item1['package_id']; ?>)"></i></span>
                                    </li>
                                    @endforeach
                                    @endif
                                 </ul>
                        ...
                        <div class="modal-dialog" style="margin-top: 12%;max-width: 50%!important;" >
                           <div class="modal-content">
                              ...
                              <div class="modal-footer">
                                 ...
                                 <button type="button" class="btn btn-sm btn-secondary" onclick="addPerson({{$item1['package_id']}});">Add</button>
                              </div>
                           </div>
                        </div>
                     </div>
           ...
           success: function(data){
            if (data.statusCode == "200") {
                  ...
                  $.each(data.data, function (key, value) {
                     ...
                     $("#listhtml"+package_id).append('<li class="list-group-item"><label>'+(key1)+'. '+value.person_first_name+'</label>  <span ><i style="float: right;" class="fa fa-trash" onclick="removePerson('+value.id+')"></i></span></li>');
                  });
                  // location.reload(true);
            }else{
                  toastr.error(data.message)
      ...
      $.ajax({
           ...
           success: function(data){
               if (data.statusCode == "200") {
                  ...
                  $.each(data.data, function (key, value) {
                     var key1 = key + parseInt(1);
                     $("#listhtml"+package_id).append('<li class="list-group-item"><label>'+(key1)+'. '+value.person_first_name+'</label>  <span ><i style="float: right;" class="fa fa-trash" onclick="removePerson('+value.id+','+package_id+')"></i></span></li>');
                  });
                  // location.reload(true);
               }else{
                  toastr.error(data.message);
